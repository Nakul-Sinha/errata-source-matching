...
1. **Rich pairwise features (OOD-stable)**  
   For each (query, candidate) pair: distinctive token coverage/weighted coverage, acronym hits, 2–3gram phrase hits, char-4gram overlap, year presence/proximity, containment, length-bucket hit counts. No corpus IDF.
