"""Bag many early-stopped feature MLPs (peak is usually epoch 1)."""
...
def main():
    ...
    for seed in range(20):
        ...
        # bootstrap rows
        boot = np.random.randint(0, len(tr), size=len(tr))
        model = nn.Sequential(
            nn.Linear(20, 128), nn.GELU(), nn.Dropout(0.2), nn.Linear(128, 64), nn.GELU(), nn.Linear(64, 1)
        ).to(device)
        opt = torch.optim.AdamW(model.parameters(), lr=2e-3, weight_decay=0.03)
        # train exactly 1 epoch (peak)
        model.train()
        idx = np.random.permutation(boot)
        ...
        model.eval()
        with torch.no_grad():
            logits.append(model(torch.tensor(Xva, device=device)).squeeze(-1).cpu().numpy())
        if (seed + 1) % 5 == 0:
            ...
            print(f"seeds={seed+1} {m}", flush=True)
